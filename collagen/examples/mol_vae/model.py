...
class CustomGraphConv(MessagePassing):
    def __init__(self, latent_size: int, num_layers: int, edge_size: int,
        ...
        self.weight = nn.Parameter(torch.Tensor(num_layers, edge_size, latent_size, latent_size))
    ...
    def message(self, n, x_j, bond_idx):
        ...
        return torch.bmm(mat, x_j.view((-1,self.latent_size,1))).view((-1,self.latent_size))
...
class MolVAE(pl.LightningModule):
    def __init__(self, atom_dim: int, bond_dim: int, z_size: int, z_select: int, 
                 z_bond: int, z_atom: int, dist_r: int, enc_steps: int, dec_steps: int,
                 use_argmax: bool, kl_warm: int, kl_ramp: int, kl_hold: int, kl_max: float, image_freq: int = 50):
        super().__init__()
    ...
    def predict_atom_types(self, z_init: Tensor) -> Tensor:
        ...
        tau_init = F.one_hot(
            ...
            self.hparams['atom_dim'])

        return (tau_init, scores)

    def predict_edge(self, phi: Tensor, mask: Tensor, train: bool = False) -> Optional[int]:
        edge_mask = torch.cat([
            torch.tensor(1, device=self.device).reshape((1,)), 
            mask
        ], axis=0)
    ...
    def predict_bond(self, phi: Tensor, avail_valency: dict, focus: int, target: int) -> int:
        bond_max = min(avail_valency[focus], avail_valency[target])
        bond_mask = torch.tensor([1, int(bond_max >= 2), int(bond_max >= 3)], device=self.device)
    ...
    def build_phi(self, G: GraphMol, t: int, u: int, H_0: Tensor, N: int) -> Tensor:
        ...
        s_v = torch.cat([
            torch.zeros((1,Z), device=self.device),
            G.x
        ], axis=0)
        phi_r = torch.cat([
    ...
    def decode(self, z_init: Tensor, train_ref: Optional[GraphMol] = None):
        ...
        if is_train:
            loss['atom'] = self.mse(
                torch.sum(atom_scores, dim=0),
                torch.sum(train_ref.x, dim=0)
            )
            acc['atom'] = torch.mean(
                (torch.argmax(atom_types, dim=1) == torch.argmax(train_ref.x, dim=1)).float()
            )
            ...
            if target is None:
                ...
                if is_train:
                    loss['select'] += self.bce(
                        edge_scores,
                        util.categorical([0], N+1, device=self.device)
                    )
                    acc['select'] += (
                        int(pred_target == target)
                    )
                    num_edge += 1
            else:
                # New edge.
                ...
                if is_train:
                    loss['select'] += self.bce(
                        edge_scores,
                        util.categorical([x+1 for x in neighbors], N+1, device=self.device)
                    )
                    acc['select'] += (
                        int(pred_target is not None and pred_target in neighbors)
                    )
                    num_edge += 1
                
                # Predict bond type.
                bond_pred, bond_scores = self.predict_bond(phi, avail_valency, focus, target)
        ...
        if G.edge_index.shape[1] == 0:
            ...
            res = GraphMol(x=atom_types[valid], edge_index=edge_index, edge_attr=edge_attr)
