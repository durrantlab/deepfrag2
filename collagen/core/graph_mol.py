...
class GraphMol(Data):
    ...
    def add_edge(self, a: int, b: int, attr: Tensor):
        if self._edge_alloc >= self._edge_index.shape[1]:
            self._edge_index = torch.cat([
                self._edge_index,
                torch.empty_like(self._edge_index)
            ], dim=1)
            
            self._edge_attr = torch.cat([
                self._edge_attr,
                torch.empty_like(self._edge_attr)
            ], dim=0)
    ...
    def edge_index(self, value: Tensor):
        ...
        if self._edge_index is None or size > self._edge_index.shape[1]:
            self._edge_index = torch.empty((2,size), device=value.device, dtype=torch.long)
    ...
    def edge_attr(self, value: Tensor):
        ...
        if self._edge_attr is None or size > self._edge_attr.shape[0]:
            self._edge_attr = torch.empty((size,value.shape[1]), device=value.device, dtype=value.dtype)
    ...
    def random_trace(self):
        ...
        while len(queue) > 0:
            ...
            if len(neighbors) == 0:
                ...
                trace.append((focus, neighbors, pick, self.edge_attr[attr_lookup[(focus,pick)]]))
                adj[focus,pick] = 0
                adj[pick,focus] = 0
