...
def do_batch(mod: MolAutoencoder, loss_fn, batch: List[MolGraph], mask_ratio: float):
    ...
    return {"acc": acc, "atom_acc": atom_acc, "loss": loss}
...
def train(attr: dict, save_path: str, wandb_project: str, cpu_only: bool):
    ...
    loader = DataLoader(
        dat, batch_size=attr["batch_size"], shuffle=True, collate_fn=lambda x: x
    )
    ...
    opt = torch.optim.Adam(
        list(mod.models["encoder"].parameters())
        + list(mod.models["decoder"].parameters()),
        lr=float(attr["lr"]),
    )

    if wandb_project is not None:
        wandb.init(project=wandb_project, reinit=True, config=attr)
    ...
    for epoch in range(attr["num_epochs"]):
        ...
        for idx, batch in tqdm(
            enumerate(loader), desc=f"Epoch {epoch}", total=num_steps
        ):
            curr_step += 1
...
def main():
    ...
    parser.add_argument(
        "--save_path", required=True, help="Path to target run directory."
    )
    parser.add_argument(
        "--wandb",
        required=False,
    ...
    train(attr, save_path=args.save_path, wandb_project=args.wandb, cpu_only=args.cpu)
